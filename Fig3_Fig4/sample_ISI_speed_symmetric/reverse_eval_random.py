...
def sample_eval(q, rand_seed, sample_in_process, spike_num, STDPtype, STDPcombi):
    ...
    bias_hist=numpy.zeros([sample_in_process, 7])
    for nsamp in range(sample_in_process):
        #random parameters
        meanISI=5.0+45.0*numpy.random.rand()
        ms_per_neuron=5.0+45.0*numpy.random.rand()
        ...
        bias_hist[nsamp,:]=[meanISI, ms_per_neuron, numpy.mean(deltaw_bias), wilcox[0], wilcox[1], len(deltaw_bias[deltaw_bias>0.0])/len(deltaw_bias), binom]
