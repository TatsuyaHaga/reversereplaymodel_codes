...
pylab.ylim([0.0, 0.4])
pylab.yticks([0.0, 0.4])
pylab.xticks([])
pylab.legend()

pylab.subplot2grid((3,1), (2,0), rowspan=1)
pylab.plot(xarr, wchange[xarr], label="change", color="blue")
pylab.plot(xarr, [0]*len(xarr), "--", color="black")
pylab.plot(pre_neuron, 0, "o", color="black")
pylab.ylim([numpy.around(numpy.min(wchange[xarr]), decimals=2)-0.01, numpy.around(numpy.max(wchange[xarr]), decimals=2)+0.01])
pylab.yticks([0.0, numpy.around(numpy.max(wchange[xarr]), decimals=2)+0.01])
pylab.xlabel("Neuron #")
pylab.ylabel("Change")
